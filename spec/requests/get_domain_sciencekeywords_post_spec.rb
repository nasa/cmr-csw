...
RSpec.describe 'GetDomain http POST (ScienceKeywords queryable) success scenarios', :type => :request do
...
<GetDomain
   ...
   xsi:schemaLocation="http://www.opengis.net/cat/csw/2.0.2 CSW-discovery.xsd">
    <PropertyName>ScienceKeywords</PropertyName>
</GetDomain>
    eos
    ...
    expect(domain_xml.root.at_xpath('/csw:GetDomainResponse/csw:DomainValues/csw:PropertyName',
                                    'csw' => 'http://www.opengis.net/cat/csw/2.0.2').text).to eq('ScienceKeywords')
    ...
    expect(domain_xml.root.at_xpath('/csw:GetDomainResponse/csw:DomainValues/csw:ConceptualScheme/csw:Document',
                                    'csw' => 'http://www.opengis.net/cat/csw/2.0.2').text).to eq('http://gcmdservices.gsfc.nasa.gov/static/kms/sciencekeywords/sciencekeywords.csv')
  ...
  it 'correctly renders the response for both ScienceKeywords and TempExtent_end properties' do
    post_xml = <<-eos
<?xml version="1.0" encoding="ISO-8859-1"?>
<GetDomain
   service="CSW"
...
RSpec.describe 'GetDomain http POST (ScienceKeywords queryable) error scenarios', :type => :request do
