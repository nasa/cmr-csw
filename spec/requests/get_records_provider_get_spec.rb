RSpec.describe "various successful GetRecords GET requests with the Provider constraint", :type => :request do

  it 'correctly renders RESULTS FULL ISO MENDS (gmi) data in response to a Provider IsGeoss constraint GET request' do
    VCR.use_cassette 'requests/get_records/gmi/larc_prov_1', :decode_compressed_response => true, :record => :once do
      get '/collections', :params => { :request => 'GetRecords', :service => 'CSW', :version => '2.0.2', :ElementSetName => 'full' , :resultType => 'results', :constraint => 'Provider=LARC and IsGeoss=true', :CONSTRAINTLANGUAGE => 'CQL_TEXT' }
      ...
      expect(search_results_node_set[0]['numberOfRecordsMatched']).to eq('224')
  ...
  it 'correctly renders HITS data in response to a Provider IsGeoss constraint and resultType HITS GET request' do
    VCR.use_cassette 'requests/get_records/gmi/larc_prov_2', :decode_compressed_response => true, :record => :once do
      get '/collections', :params => { :request => 'GetRecords', :service => 'CSW', :version => '2.0.2', :ElementSetName => 'full',
          :constraint => 'Provider=LARC and IsGeoss=true', :CONSTRAINTLANGUAGE => 'CQL_TEXT', :resultType => 'hits' }
      expect(response).to have_http_status(:success)
      expect(response).to render_template('get_records/index.xml.erb')
      records_xml = Nokogiri::XML(response.body)
      expect(records_xml.root.name).to eq 'GetRecordsResponse'
