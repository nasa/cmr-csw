RSpec.describe 'GetDomain http POST "IsCwic" queryable success scenarios', :type => :request do
...
<GetDomain
   ...
   xsi:schemaLocation="http://www.opengis.net/cat/csw/2.0.2 CSW-discovery.xsd">
    ...
    expect(domain_xml.root.at_xpath('/csw:GetDomainResponse/csw:DomainValues/csw:PropertyName',
                                    'csw' => 'http://www.opengis.net/cat/csw/2.0.2').text).to eq('IsCwic')
  ...
  it 'correctly renders the response for IsCwic and an additional property' do
    post_xml = <<-eos
<?xml version="1.0" encoding="ISO-8859-1"?>
<GetDomain
   service="CSW"
