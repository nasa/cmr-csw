...
RSpec.describe 'GetDomain http POST (ArchiveCenter queryable) success scenarios', :type => :request do
...
<GetDomain
   ...
   xsi:schemaLocation="http://www.opengis.net/cat/csw/2.0.2 CSW-discovery.xsd">
    <PropertyName>ArchiveCenter</PropertyName>
</GetDomain>
    eos
    ...
    expect(domain_xml.root.at_xpath('/csw:GetDomainResponse/csw:DomainValues/csw:PropertyName',
                                    'csw' => 'http://www.opengis.net/cat/csw/2.0.2').text).to eq('ArchiveCenter')
  ...
  it 'correctly renders the response for both ArchiveCenter and TempExtent_end properties' do
    post_xml = <<-eos
<?xml version="1.0" encoding="ISO-8859-1"?>
<GetDomain
   service="CSW"
...
RSpec.describe 'GetDomain http POST (ArchiveCenter queryable) error scenarios', :type => :request do
