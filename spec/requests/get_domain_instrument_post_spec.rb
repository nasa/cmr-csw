...
RSpec.describe 'GetDomain http POST (Instrument queryable) success scenarios', :type => :request do

  it 'correctly renders the response for a the Instrument PropertyName' do
    post_xml = <<-eos
...
<GetDomain
  ...
  it 'correctly renders the response for both Instrument and TempExtent_end properties' do
    post_xml = <<-eos
<?xml version="1.0" encoding="ISO-8859-1"?>
<GetDomain
   service="CSW"
...
RSpec.describe 'GetDomain http POST (Instrument queryable) error scenarios', :type => :request do
