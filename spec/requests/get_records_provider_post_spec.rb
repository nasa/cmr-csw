RSpec.describe "various GetRecords POST requests based on the Provider queryable", :type => :request do

  it 'correctly renders FULL RESULTS ISO MENDS (GMI) data in response to an Provider IsGeoss constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/larc_4', :decode_compressed_response => true, :record => :once do
...
<csw:GetRecords xmlns:csw="http://www.opengis.net/cat/csw/2.0.2" resultType="results"
    ...
    <csw:Query typeNames="csw:Record">
      ...
      expect(search_results_node_set[0]['numberOfRecordsMatched']).to eq('15456')
  ...
  it 'correctly renders HITS data in response to an ArchiveCenter IsGeoss constraint and resultType HITS POST request' do
    VCR.use_cassette 'requests/get_records/gmi/larc_5', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
